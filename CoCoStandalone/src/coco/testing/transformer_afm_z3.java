...
public class transformer_afm_z3 {
	...
	public static HashMap <String,IntExpr> Features = new HashMap();
	...
	public transformer_afm_z3() {
		...
		opt = ctx.mkOptimize();
		transformer_afm_z3.totalizador_SC = ctx.mkIntConst("TOTALIZADOR");
	...
	private static void generate_constraints(String type, String right, String left) {
		if(type.trim().equals("REQUIRES")) {
			TreeStructure.add(ctx.mkImplies(ctx.mkEq(Features.get(left), ctx.mkInt(1)),
					ctx.mkEq(Features.get(right), ctx.mkInt(1))));
		}else if(type.trim().equals("EXCLUDES")) {
			TreeStructure.add(ctx.mkNot(ctx.mkAnd(ctx.mkEq(Features.get(left), ctx.mkInt(1)),
					ctx.mkEq(Features.get(right), ctx.mkInt(1)))));
		}
	}

	private static void getAttributes(int sols_exp)  {
		if (sols_exp > 0) {
			Attributes = new ArrayList();
		}
	...
	private static void generate_attributes_generic() {
		...
		for (Map.Entry<String, ArrayList> entry : Attributes_Gen.entrySet()) {
			...
			if(optimizations_hash.containsKey(Atts.get(entry.getKey())+"".trim())) {
				...
				arrtemp1.add(Atts.get(entry.getKey()));
			}
			//solver.MkMinimize(Atts.get(entry.getKey()));
			index++;
			
		}
		IntExpr[] arrtemp2 = new IntExpr[arrtemp1.size()];
		for(int x=0;x<arrtemp1.size();x++) {
			arrtemp2[x] = arrtemp1.get(x);
			  //System.out.println(arrtemp1.get(x));
			}
	...
	private static IntExpr generate_attribute(String attr_feat, String attr_gen, String mindom, String maxdom) {
		...
		try {
			...
			IntExpr feature = Features.get(attr_feat);
			if(feature != null) {
			BoolExpr imp1 = ctx.mkImplies(ctx.mkEq(feature, ctx.mkInt(1)), q);
			BoolExpr imp2 = ctx.mkImplies(ctx.mkEq(feature, ctx.mkInt(0)), ctx.mkEq(sss, ctx.mkInt(0)));
	...
	private static void alternative_or(String type_gate, ArrayList<IntExpr> involvedFeatures,  IntExpr id_padre) {
		if(type_gate.equals("alternative")){
			
			IntExpr parent = id_padre;
			IntExpr[] children = new IntExpr[involvedFeatures.size()];
			IntExpr sum = ctx.mkIntConst("SUM_OR_"+id_padre.toString().replaceAll("\\|", ""));
			for (int j = 0; j < children.length; j++) {
				children[j] = involvedFeatures.get(j );// features.get(involvedFeatures.get(j
			}
			ArithExpr tmp1s =ctx.mkAdd(children); 
			//solver.Assert(ctx.mkEq(sum,tmp1s ));
			TreeStructure.add(ctx.mkEq(tmp1s,sum ));
			//BoolExpr imp1 = ctx.mkImplies(ctx.mkEq(parent, ctx.mkInt(1)), ctx.mkEq(sum, ctx.mkInt(0)));
			//BoolExpr imp2 = ctx.mkImplies(ctx.mkEq(parent, ctx.mkInt(0)), ctx.mkEq(sum, ctx.mkInt(0)));
			//solver.Assert(ctx.mkAnd(imp1,imp2));
			TreeStructure.add(ctx.mkEq(sum ,parent));
			/*
			...
			*/
			...
			ArithExpr tmp1s =ctx.mkAdd(children); 
			//solver.Assert(ctx.mkEq(sum,tmp1s ));
			TreeStructure.add(ctx.mkEq(sum,tmp1s ));
			BoolExpr imp1 = ctx.mkImplies(ctx.mkEq(parent, ctx.mkInt(1)), ctx.mkGt(sum, ctx.mkInt(0)));
			BoolExpr imp2 = ctx.mkImplies(ctx.mkEq(parent, ctx.mkInt(0)), ctx.mkEq(sum, ctx.mkInt(0)));
			//solver.Assert(ctx.mkAnd(imp1,imp2));
			TreeStructure.add(ctx.mkAnd(imp1,imp2));
	...
	private static void optional_mandatory(String mandatory, IntExpr id_padre, IntExpr id_hijo) {
		...
		if (mandatory == "mandatory") {
			try {
				...
				TreeStructure.add(
						ctx.mkEq(id_hijo,id_padre));//,ctx.mkInt(1)),
								//ctx.mkEq(, ctx.mkInt(1))));
			} catch (Exception e) {
				e.printStackTrace();
			...
			try {
				...
				*/
				TreeStructure.add(
						ctx.mkLe(id_hijo,id_padre));
				/*TreeStructure.add(ctx.mkImplies(ctx.mkEq(id_hijo, ctx.mkInt(1)),
						ctx.mkEq(id_padre, ctx.mkInt(1))));*/
			} catch (Exception e) {
				System.out.println("constraint paila:" + id_padre + "::::" + id_hijo);
			}
		}
	...
	private static IntExpr root(String name) {
		...
		try {
			...
			if (!Features.containsKey(name)) {
				IntExpr x = ctx.mkIntConst(name);
				BoolExpr low  = ctx.mkGe((ArithExpr)x,(ArithExpr)ctx.mkInt(0));
				BoolExpr high = ctx.mkLe((ArithExpr)x,(ArithExpr)ctx.mkInt(1));
				solver.Assert(ctx.mkAnd(low,high));
				
				TreeStructure.add(ctx.mkEq(x, ctx.mkInt(1)));
				Features.put(name, x);
				return x;
			} else {
...
*/
	private static IntExpr not_root(String name) {
		int nwindx = getIndex();
		name = "(" + namedef + ")_" + name;
